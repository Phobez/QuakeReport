...
object QueryUtils {
    ...
    private const val EMPTY_JSON_RESPONSE = "{\"type\":\"FeatureCollection\",\"metadata\":{\"generated\":1462295443000,\"url\":\"http://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=2016-01-01&endtime=2016-01-31&minmag=6&limit=10\",\"title\":\"USGS Earthquakes\",\"status\":200,\"api\":\"1.5.2\",\"limit\":10,\"offset\":1,\"count\":10},\"features\":[]}"
    ...
    suspend fun extractEarthquakes(): ArrayList<Earthquake> {
        ...
        try {
            ...
            // arse the response string and
            // build up a list of Earthquake objects with the corresponding data.
            val res = JSONObject(jsonResponse)
            val features = res.getJSONArray("features")

            val maxIndex = features.length() - 1

            for (i in 0..maxIndex) {
                val earthquake = features.getJSONObject(i)
                val properties = earthquake.getJSONObject("properties")
                val magnitude = properties.getDouble("mag")
                val location = properties.getString("place")
                val time = properties.getLong("time")
                val earthquakeUrl = properties.getString("url")
                val newEarthquake = Earthquake(magnitude, location, time, earthquakeUrl)
